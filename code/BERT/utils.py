...
def evaluate_auc(id_to_tag, y_pred, id_to_word,test_tags,test_words, dataset):
    ...
    fpr["micro"], tpr["micro"], _ = roc_curve(true_list.ravel(), pred_list.ravel())
    ...
    colors = cycle(['aqua', 'darkorange', 'cornflowerblue', 'plum', 'purple', 'red'])
    for i, color in zip(range(n_classes), colors):
        plt.plot(fpr[i], tpr[i], color=color, lw=lw,
                 label='ROC curve of class {0} (area = {1:0.2f})'
                       ''.format(id_to_tag[i], roc_auc[i]))
    ...
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title('multi-class ROC results')
    plt.legend(loc="lower right")
    plt.savefig('roc_auc_full_'+dataset)
